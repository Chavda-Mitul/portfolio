...
const replacements = {
 ...
 CONTACTOPTIONS: ['Email','Book a call','LinkedIn'].map(label=>socials.find(s=>s.label===label)).filter(Boolean).map(s=>`<a href="${escape(s.url)}" ${s.url.startsWith('https')?'target="_blank" rel="noopener noreferrer"':''}>${socialIcon(s.label)}<span><strong>${escape(s.label==='Email'?'Email me':s.label==='LinkedIn'?'Message on LinkedIn':s.label)}</strong><small>${escape(s.label==='Email'?resume.email:s.label==='Book a call'?'Choose a time on my calendar':'Connect or send me a message')}</small></span></a>`).join(''),
  ...
  HOMECONTACT: `<div class="home-contact-links">${['Book a call','Email','LinkedIn'].map(label=>{const item=socials.find(s=>s.label===label);return `<a href="${escape(item.url)}"${item.url.startsWith('https')?' target="_blank" rel="noopener noreferrer"':''}>${socialIcon(label)}<span>${label==='Email'?'Email me':label}</span></a>`;}).join('')}</div>`,
  CONTACTPAGE: (()=>{
...
<div class="contact-directory"><section aria-labelledby="reach-title"><h2 id="reach-title">Get in touch</h2><ul>${['Book a call','Email','LinkedIn','Résumé'].map(contactLink).join('')}</ul></section><section aria-labelledby="elsewhere-title"><h2 id="elsewhere-title">Elsewhere</h2><ul>${['GitHub','YouTube','X','Instagram'].map(contactLink).join('')}</ul></section></div>
<div class="contact-details"><dl class="contact-facts"><div><dt>Based in</dt><dd>India · IST (UTC+5:30)</dd></div><div><dt>Looking for</dt><dd>AI engineering and full-stack roles, freelance builds</dd></div><div><dt>Currently</dt><dd>AI Engineering intern at withlayer.ai</dd></div></dl></div></section>`;
  ...
  HEATMAP: (()=>{
    ...
    return `<section class="github-activity" aria-labelledby="github-title"><h2 id="github-title">Building in public</h2><p>${total.toLocaleString('en')} contributions in this GitHub snapshot.</p><svg viewBox="0 0 ${weeks*13} 113" role="img" aria-label="GitHub contribution activity from ${days[0].date} to ${days.at(-1).date}">${months}${cells}</svg><div class="heatmap-footer"><a href="https://github.com/Mitul Chavdas" target="_blank" rel="noopener noreferrer">View GitHub activity</a><span>Updated ${data.updated}</span><span class="heat-legend">Less ${[0,1,2,3,4].map(n=>`<i style="background:var(--heat-${n})"></i>`).join('')} More</span></div></section>`;
  })(),
  ROLES: roleMarkup,
