...
for(const p of projects){
 ...
 if(p.url)assert.ok(p.url.startsWith('https://github.com/Mitul Chavdas/'),`${p.name} source on my GitHub`);
}
...
assert.equal((html.match(/data-cal-link="Mitul Chavdas"/g)||[]).length,0,'inline calendar removed');
